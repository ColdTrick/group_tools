...
if (!empty($user)) {
	...
	if (!empty($group_membership)) {
		...
		$text = elgg_echo('group_tools:widgets:start_discussion:membership_required', [$link_start, $link_end]);
	...
	echo elgg_view('output/longtext', [
		'value' => elgg_echo('group_tools:widgets:start_discussion:login_required'),
	]);
}

echo elgg_format_element('script', ['type' => 'text/javascript'], 'require(["group_tools/start_discussion"]);');
